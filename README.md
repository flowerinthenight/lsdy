...
The output uses Go's builtin [tabwriter](https://golang.org/pkg/text/tabwriter/) instead of the more sophisticated terminal-based UI libraries (i.e. [tcell](https://github.com/gdamore/tcell)) for easy piping into other cmdline tools (i.e. `grep`, `awk`, etc).
...
```
_Warning!_ At the moment, `--describe` will cause a table scan if the `--pk` flag is not set. If your table is huge, it's probably a good idea to supply the `--pk` flag.
